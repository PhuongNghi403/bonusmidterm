...
class CurriculumVisualizer(QMainWindow):
    def __init__(self):
        ...
        self.openButton.clicked.connect(self.open_chart_in_browser)
        self.saveButton.clicked.connect(self.save_chart_to_html)
    ...
    def create_sunburst_chart(self, df):
        """Tạo biểu đồ sunburst từ DataFrame với màu sắc đẹp hơn"""
        try:
            # Tạo bảng màu tùy chỉnh
            custom_colors = [
                '#8e44ad', '#9b59b6', '#2ecc71', '#27ae60', '#3498db',
                '#2980b9', '#e74c3c', '#c0392b', '#f1c40f', '#f39c12',
                '#1abc9c', '#16a085', '#d35400', '#e67e22', '#95a5a6',
                '#7f8c8d', '#34495e', '#2c3e50', '#8e44ad', '#9b59b6'
            ]

            # Tạo biểu đồ sunburst với màu sắc tùy chỉnh
            fig = px.sunburst(
                df,
                ids='id',
                parents='parent',
            ...
            # Tùy chỉnh biểu đồ với màu nền tối
            fig.update_layout(
                margin=dict(t=30, l=0, r=0, b=0),
                font=dict(size=14, family="Arial, sans-serif", color="#ecf0f1"),
                height=800,
                width=1000,
                title_font_size=24,
